...
import { getSessionPrompt, getAIResponse, sleep } from "./ai-interactions-3.ts";
import { MODEL_NAME } from "./ai-settings-3.ts";
import {
  type SessionData,
  buildHeader,
  ...
  for (const dataset of objectNotation.datasets) {
    ...
    for (let i = 0; i < questionKeys.length; i++) {
      ...
      try {
        ...
        console.log(`Session ${sessionNumber}: Sending request to Local API...`);
